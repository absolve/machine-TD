...
func refresh() -> void:
	...
	if expBar:
		...
		expBar.value = clamp(exp, 0, expBar.max_value)
	if hpLabel:
	...
	if expLabel:
		if exp_needed <= 0:
			...
			expLabel.text = "%d/%d" % [exp, exp_needed]
	visible = selected

func set_status(_hp: int, _max_hp: int, _exp: int, _exp_needed: int, _selected: bool = false) -> void:
	hp = _hp
