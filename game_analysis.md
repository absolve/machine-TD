...
所有关卡统一使用 30×16 网格，单格大小为 64 px，地图尺寸为 1920×1024 px，确保与现有的 `base_level.gd` 与 `TileSize` 规则完全一致。
坐标原点在地图左上角，坐标格式为 `(x, y)`，路线节点和塔位均使用网格坐标。

- **地图尺寸**：`GridCols = 30`，`GridRows = 16`，`MapSize = Vector2i(1920, 1024)`。
- **关卡类型**：每一关都声明一个地图类型，如草地、工厂、沙漠、废墟、雪地等，便于选关界面和缩略图区分。
- **敌人入口**：默认从左侧进入，常见入口有 `(0,2)`、`(0,7)`、`(0,12)`；多个入口最终必须汇入统一基地。
...
- **分流原则**：前 3 关偏教学型单路和双路；4 至 6 关加入工厂/沙漠场景与中段合流；7 至 10 关加入多入口与三路压制；11 至 15 关升级为冰雪、废墟、火山和终局防线。

图例：`S` 入口，`R1/R2` 路线编号，`W1/W2` 表示 1 格/2 格路线，`J` 合流点，`B` 基地，`T` 普通塔位，`L` 大型塔位，`-` 不可建造区域。

#### 2.7.2 重设后的地图类型与路线总览

关卡现在按“地形主题 + 简化路线拓扑”进行设计，保证玩家在关卡选择界面能一眼辨识，而且战斗地图很容易做成简单的缩略图。

| 关卡 | 地图类型 | 路线设计 | 地图特征 | 关卡想法 |
|------|----------|----------|----------|----------|
| 1 | 草地训练场 | 单直线 `入口 → 直线 → 基地` | 低难度、无分叉 | 教学关，先教放塔和拆塔 |
| 2 | 草地仓储区 | S 弯 `入口 → 右转 → 直线 → 基地` | 一小段转弯、轻微压制 | 训练转角塔位和补刀 |
| 3 | 双坡谷 | 双线路 `上路 / 下路` 中段合流 | 中段有桥头，前后分区 | 练习前线与后排分工 |
| 4 | 工厂高速道 | 双路高速、中央合流 | 宽度较大、快速推进 | 适合减速塔和群伤塔 |
| 5 | 导弹工厂 | 一路绕箱体、一路窄道 | 中段回折、末端较窄 | 喜欢压线和后排炮台 |
| 6 | 沙漠哨站 | 三入口，中央汇合 | 空地混合，节奏更快 | 练习多路线管理 |
| 7 | 双门要塞 | 双门分离后再次汇合 | 视野清晰、合流可控 | 适合分区防守 |
| 8 | 环城高架 | 上下环绕后转入基地 | 回环路线，节奏明显 | 需要中后段高火力 |
| 9 | 三路峡口 | 三路折返、前中后分层 | 视野复杂但不混乱 | 强调层次压制 |
| 10 | 终局核心区 | 三路汇入最终合流 | 最终前线非常关键 | 需要全地图火力覆盖 |
| 11 | 冰雪驿站 | 双路压缩入核心 | 低视野、偏压线 | 训练进攻节奏控制 |
| 12 | 废弃城墙 | 绕城后集中收束 | 路线偏长、夹角大 | 打法更保守些 |
| 13 | 火山要塞 | 两路高压收束 | 中后段窄路 | 强制前后排搭配 |
| 14 | 钢铁防线 | 网格式双路推进 | 线性但多拐角 | 要求布局稳定 |
| 15 | 深空核心 | 三路冲击、末端总汇 | 最终大boss前的压榨 | 终局阶段，整体战术高度 |

#### 2.7.3 各关卡路线缩略图（简化版）

为了让关卡选择界面更清晰，路线缩略图统一使用“入口 / 路线 / 合流 / 基地”四个结构化元素，尽量避免复杂节点。缩略图只保留主路线、转折点和基地，不展示细碎塔位与敌人数量。
...
建议 `StageData` 中新增字段：`terrain`、`map_type`、`route_style`、`routes`、`thumbnail`，并让地图预览和实际战斗地图共用同一份数据来源，这样不会出现文档、UI 和关卡本体路线不一致的问题。
