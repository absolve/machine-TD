...
- **深银金属主题**（2026-09-13 由亮色 v1.0 重建，v2.0）：调色板从参考资源包
  ...
  化学绿/锈红/青语义色。**地板 `#333C57` / 面板 `#566C86` / 亮边框 `#94B0C2`**
  三层明度（0.046 / 0.144 / 0.412），靠明度分层而非投影。
  背景基色与 `rendering/environment/defaults/default_clear_color` 同为 `#333C57`，
  全场景底色一致，砖格 64px 对齐建造网格。
